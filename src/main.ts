  ...
  try {
    ...
    const ok = (
      ...
      rr.forEach((r) => {
        ...
        r.compoundsLemma = trimAndIgnoreEmpty(
          uniq(r.compoundsLemma.map(stripHtml)),
        );
    ...
    if (req.method === "GET") {
      ...
      if (cached) {
        console.log(
          ...
          }: GET (from LRU cache) /analyse?sourceLanguage=:${sourceLanguage}, word=: ${word}`,
        );
      ...
      console.log(
        `${
          new Date().toISOString()
        }: GET /analyse?sourceLanguage=:${sourceLanguage}, word=: ${word}`,
      );
      ...
      const saolResults: WordQueryResponse[] = saolHits.map((hit: any) => ({
        ...
        compoundsLemma: [
          ...
          ...(hit._source.enbartDigitalaHänvisningar || []).map(
            parseLemmaFromHtml,
          ),
        ],
...
console.log(
  `${
    new Date().toISOString()
  }: Link for debugging: http://localhost:${port}/analyse?word=anden`,
);
console.log(
  `${
    new Date().toISOString()
  }: Link for debugging: http://localhost:${port}/analyse?sourceLanguage=it&word=di`,
);
console.log(
  `${
    new Date().toISOString()
  }: Link for debugging: http://localhost:${port}/analyse?sourceLanguage=it&word=offrirvi`,
);
